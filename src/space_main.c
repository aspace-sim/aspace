...
return_t crack_list(char *clist, int min_list, int max_list, int actual, array_t array)
{
  int i = 0;
  int j, p;
...
FUNCTION(local_fun_debug_xyz2vis)
{
   
   double dx2 = -28233785541430.449219;
   double dy2 = 117255065636.541763;
   double dz2 = -24685276976.114056;
   double px = dx2 / PARSEC;
   double py = dy2 / PARSEC;
   double pz = dz2 / PARSEC;
   double dx = fabs(px - (round3(px / 100.0) * 100.0));
   double dy = fabs(py - (round3(py / 100.0) * 100.0));
   double dz = fabs(pz - (round3(pz / 100.0) * 100.0));
   double vis;
   
   vis = 1.1 - (1.0 / (1.0 + dx * dx + dy * dy + dz * dz));
   
   safe_str("VIS: ", buff, bp);
   safe_str(unparse_number(vis), buff, bp);
   safe_str("\n", buff, bp);
   
   safe_str("PX: ", buff, bp);
   safe_str(unparse_number(px), buff, bp);
   safe_str("\n", buff, bp);
   
   safe_str("PY: ", buff, bp);
   safe_str(unparse_number(py), buff, bp);
   safe_str("\n", buff, bp);
   
   safe_str("PZ: ", buff, bp);
   safe_str(unparse_number(pz), buff, bp);
   safe_str("\n", buff, bp);
   
   safe_str("DX: ", buff, bp);
   safe_str(unparse_number(dx), buff, bp);
   safe_str("\n", buff, bp);
   
   safe_str("DY: ", buff, bp);
   safe_str(unparse_number(dy), buff, bp);
   safe_str("\n", buff, bp);
   
   safe_str("DZ: ", buff, bp);
   safe_str(unparse_number(dz), buff, bp);
   safe_str("\n", buff, bp);
}
...
FUNCTION(fun_isdark)
{
	...
	player = parse_dbref(args[0]);

	if (!GoodObject(player)) {
		safe_str("#-1 INVALID PLAYER", buff, bp);
	}

	if ((has_flag_by_name(player, "WIZARD", NOTYPE) || has_flag_by_name(player, "ROYALTY", NOTYPE) || has_flag_by_name(player, "DUKE", NOTYPE) || has_flag_by_name(player, "DARK", NOTYPE) || hidden(player)) && !has_flag_by_name(player, "LIGHT", NOTYPE))
	{
		safe_str("1", buff, bp);
	}
	else
...
{
   function_add("ISDARK", fun_isdark, 1, 1, FN_REG);
   function_add("DBG_XYZ2VIS", local_fun_debug_xyz2vis, 0, 0, FN_REG);
