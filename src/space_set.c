...
int do_set_fire (int first, int last, int weapon, int mode, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no %ss.", Name(sdb[n].object), system_name[3]);
		return 0;
	} else if (weapon == 2 && !sdb[n].missile.exist) {
		notify_format(enactor, "%s has no %ss.", Name(sdb[n].object), system_name[9]);
		return 0;
	} else if (!weapon && !sdb[n].beam.exist && !sdb[n].missile.exist) {
		notify_format(enactor, "%s has no weapons.", Name(sdb[n].object));
		return 0;
	} else if (sdb[n].status.docked) {
		notify_format(enactor, "%s is in dock.", Name(sdb[n].object));
		return 0;
	} else if (sdb[n].status.landed) {
		notify_format(enactor, "%s is on a landing pad.", Name(sdb[n].object));
		return 0;
	} else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
		notify_format(enactor, "%s cannot fire weapons while cloaked.", Name(sdb[n].object));
		return 0;
	}
	...
	if (!weapon && !is_b_load && !is_m_load) {
		if (!is_b_active) {
			notify_format(enactor, "No %ss are online.", system_name[3]);
		} else if (!is_b_lock) {
			notify_format(enactor, "No %ss are locked.", system_name[3]);
		} else if (!is_b_arm) {
			notify_format(enactor, "No %ss are powered.", system_name[3]);
		} else if (!is_b_arc) {
			notify_format(enactor, "No %ss have targets in firing arc.", system_name[3]);
		} else if (!is_b_range) {
			notify_format(enactor, "No %ss have targets in range.", system_name[3]);
		} else if (!is_b_load) {
			notify_format(enactor, "No %ss are recycled.", system_name[3]);
		}
		if (!is_m_active) {
			notify_format(enactor, "No %ss are online.", system_name[9]);
		} else if (!is_m_lock) {
			notify_format(enactor, "No %ss are locked.", system_name[9]);
		} else if (!is_m_arm) {
			notify_format(enactor, "No %ss are powered.", system_name[9]);
		} else if (!is_m_arc) {
			notify_format(enactor, "No %ss have targets in firing arc.", system_name[9]);
		} else if (!is_m_range) {
			notify_format(enactor, "No %ss have targets in range.", system_name[9]);
		} else if (!is_m_load) {
			notify_format(enactor, "No %ss are recycled.", system_name[9]);
		}
		for (i = 0; i < sdb[n].sensor.contacts; ++i) {
			x = sdb[n].slist.sdb[i];
			if (sdb[x].tmp.i0 == n) {
...
int do_set_main_reactor (double level, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no M/A reactor.", Name(sdb[n].object));
...
int do_set_aux_reactor (double level, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no fusion reactor.", Name(sdb[n].object));
...
int do_set_battery (double level, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no batteries.", Name(sdb[n].object));
...
int do_set_srs (int active, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s cannot use short-range sensors while cloaked.", Name(sdb[n].object));
	} else {
		if (active) {
...
int do_set_ew (int active, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no electronic warfare systems.", Name(sdb[n].object));
	} else if (sdb[n].sensor.ew_damage <= 0.0) {
		notify(enactor, ansi_red("Electronic warfare systems are inoperative."));
	} else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
		notify_format(enactor, "%s cannot use electronic warfare sensors while cloaked.", Name(sdb[n].object));
	} else {
		if (active) {
...
int do_set_cloak (int active, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no cloaking device.", Name(sdb[n].object));
		return 0;
...
int do_set_course (double yaw, double pitch, double roll, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s is still connected.", Name(sdb[n].object));
	} else if (!sdb[n].engine.warp_exist && !sdb[n].engine.impulse_exist) {
		notify_format(enactor, "%s cannot be maneuvered.", Name(sdb[n].object));
...
int do_set_eta (double speed, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no engines.", Name(sdb[n].object));
		return 0;
	}
	...
	if (speed == 0.0) {
		...
		if (speed == 0.0) {
			notify_format(enactor, "%s has no motion.", Name(sdb[n].object));
			return 0;
		}
	}
...
int do_set_speed (double speed, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no warp engines.", Name(sdb[n].object));
	} else if (!sdb[n].engine.impulse_exist && (fabs(speed) < 1.0)) {
		notify_format(enactor, "%s has no impulse engines.", Name(sdb[n].object));
...
int do_set_shield (int shield, int active, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no shields.", Name(sdb[n].object));
		return 0;
	} else if (sdb[n].status.docked && active) {
		notify_format(enactor, "%s cannot raise shields while docked.", Name(sdb[n].object));
		return 0;
	} else if (sdb[n].status.landed && active) {
		notify_format(enactor, "%s cannot raise shields while landed.", Name(sdb[n].object));
		return 0;
	} else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
		notify_format(enactor, "%s cannot raise shields while cloaked.", Name(sdb[n].object));
		return 0;
	}
	...
	for (i = a; i <= b; ++i)
		if (sdb[n].shield.damage[i] <= 0.0) {
			notify_format(enactor, "%s is inoperative.", unparse_shield(i));
		} else if (sdb[n].shield.active[i] && active) {
			notify_format(enactor, "%s is already online.", unparse_shield(i));
		} else if (!sdb[n].shield.active[i] && !active) {
			notify_format(enactor, "%s is already offline.", unparse_shield(i));
		} else {
			if (flag)
				strncat(buffer, "\n", sizeof(buffer) - 1);
			if (active) {
...
int do_set_weapon (int first, int last, int active, int weapon, dbref enactor)
{
	...
	if (weapon == 1 || (!weapon && sdb[n].beam.exist)) {
		...
		for (i = a; i <= b ; ++i)
			if (sdb[n].blist.damage[i] <= 0.0) {
				notify_format(enactor, "%s %d is inoperative.",
					unparse_beam(sdb[n].blist.name[i]), i + 1);
			} else if (active && sdb[n].blist.active[i]) {
				notify_format(enactor, "%s %d is already online.",
					unparse_beam(sdb[n].blist.name[i]), i + 1);
			} else if (!active && !sdb[n].blist.active[i]) {
				notify_format(enactor, "%s %d is already offline.",
					unparse_beam(sdb[n].blist.name[i]), i + 1);
			} else if (active) {
				sdb[n].blist.lock[i] = 0;
...
int do_lock_weapon (int first, int last, int contact, int weapon, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s cannot lock weapons while cloaked.", Name(sdb[n].object));
		return 0;
	}
	...
	if (weapon == 1 || (!weapon && sdb[n].beam.exist)) {
		...
		for (i = a; i <= b ; ++i)
			if (sdb[n].blist.damage[i] <= 0.0) {
				...
				notify_format(enactor, "%s %d is not online.",
					unparse_beam(sdb[n].blist.name[i]), i + 1);
			} else if (sdb[n].blist.lock[i]) {
				notify_format(enactor, "%s %d is already locked.",
					unparse_beam(sdb[n].blist.name[i]), i + 1);
			} else {
	...
	if (weapon == 1 || (!weapon && sdb[n].beam.exist)) {
		...
		for (i = a; i <= b ; ++i)
			if (sdb[n].blist.damage[i] <= 0.0) {
				...
				notify_format(enactor, "%s %d is already unlocked.",
					unparse_beam(sdb[n].blist.name[i]), i + 1);
			} else {
				sdb[n].blist.lock[i] = 0;
				if (!flag_b)
...
int do_set_trans (int active, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no transporters.", Name(sdb[n].object));
	} else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
		notify_format(enactor, "%s cannot use transporters while cloaked.", Name(sdb[n].object));
	} else {
		if (active) {
...
int do_set_tract (int active, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no tractor beams.", Name(sdb[n].object));
	} else if (sdb[n].cloak.active && sdb[n].tech.cloak < 2.0) {
		notify_format(enactor, "%s cannot use tractor beams while cloaked.", Name(sdb[n].object));
	} else {
		if (active) {
...
int do_set_tract_locked (int contact, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no tractor beam.", Name(sdb[n].object));
	} else if (sdb[n].status.docked) {
		notify_format(enactor, "%s is in dock.", Name(sdb[n].object));
	} else if (sdb[n].status.landed) {
		notify_format(enactor, "%s is on a landing pad.", Name(sdb[n].object));
	} else if (sdb[n].status.tractoring) {
		notify_format(enactor, "%s is already tractoring %s.", Name(sdb[n].object), Name(sdb[sdb[n].status.tractoring].object));
	} else if (sdb[n].status.tractored) {
		notify_format(enactor, "%s is being tractored by %s.", Name(sdb[n].object), Name(sdb[sdb[n].status.tractored].object));
...
int do_set_active (dbref enactor)
{
	...
	if (sdb[n].structure.type == 0) {
		...
		notify_format(enactor, "%s systems are already active.", Name(sdb[n].object));
	} else {
...
int do_set_inactive (dbref enactor)
{
	...
	if (sdb[n].structure.type == 0) {
		...
		notify_format(enactor, "%s systems are already inactive.", Name(sdb[n].object));
	} else if (sdb[n].power.total != 0) {
		notify_format(enactor, "%s power systems are still online.", Name(sdb[n].object));
	} else {
...
int do_set_docking_doors (int active, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no docking bay.", Name(sdb[n].object));
	} else {
		if (active) {
...
int do_set_landing_doors (int active, dbref enactor)
{
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s has no landing bay.", Name(sdb[n].object));
	} else {
		if (active) {
...
int do_set_dock (int contact, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s is incapable of docking.", Name(sdb[n].object));
/* check if the tractoree can dock   ok'd 8/23/00 */
	} else if (trac && !sdb[trac].structure.can_dock) {
		notify_format(enactor, "The ship you are tractoring (%s) is incapable of docking.",Name(sdb[trac].object));
	} else if (sdb[n].status.connected) {
		notify_format(enactor, "%s is still connected.", Name(sdb[n].object));
	} else if (sdb[n].status.tractored) {
		notify_format(enactor, "%s cannot dock while being tractored.", Name(sdb[n].object));
...
/*	} else if (sdb[n].status.tractoring) {
		...
		notify_format(enactor, "%s cannot dock while at warp.", Name(sdb[n].object));
	} else {
		x = contact2sdb(n, contact);
		if (error_on_contact(n, x, enactor)) {
			return 0;
			...
			notify_format(enactor, "%s does not have a dock.", Name(sdb[x].object));
		} else if (!sdb[x].status.open_docking) {
			notify_format(enactor, "%s's docking bay doors are closed.", Name(sdb[x].object));
/* check size on dock. include the tractoree! */
				...
				notify_format(enactor, "%s does not have enough room.", Name(sdb[x].object));
		    	...
		    	notify_format(enactor, "%s must lower all shields to dock.", Name(sdb[n].object));
		} else if (sdb2dissipation(x, sdb2shield(x, n)) != 0.0) {
			notify_format(enactor, "%s's facing shield must be lowered.", Name(sdb[x].object));
		} else if (sdb[x].status.tractored) {
			notify_format(enactor, "%s is being tractored.", Name(sdb[x].object));
		} else {
...
int do_set_undock (dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s is not in dock.", Name(sdb[n].object));
	} else if (sdb[n].status.connected) {
		notify_format(enactor, "%s is still connected.", Name(sdb[n].object));
...
int do_set_land (int contact, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s is incapable of landing.", Name(sdb[n].object));
	} else if (sdb[n].status.connected) {
		notify_format(enactor, "%s is still connected.", Name(sdb[n].object));
	} else if (sdb[n].status.tractored) {
		notify_format(enactor, "%s cannot land while being tractored.", Name(sdb[n].object));
	} else if (sdb[n].status.tractoring) {
		notify_format(enactor, "%s cannot land while using its tractor beam.", Name(sdb[n].object));
	} else if (fabs(sdb[n].move.out) >= 1.0) {
		notify_format(enactor, "%s cannot land while at warp.", Name(sdb[n].object));
	} else {
		x = contact2sdb(n, contact);
		if (error_on_contact(n, x, enactor)) {
			return 0;
			...
			notify_format(enactor, "%s does not have a landing pad.", Name(sdb[x].object));
		} else if (!sdb[x].status.open_landing) {
			notify_format(enactor, "%s's landing bay doors are closed.", Name(sdb[x].object));
		} else if ((sdb[n].structure.displacement + sdb[x].structure.cargo_mass) > sdb[x].structure.cargo_hold) {
			notify_format(enactor, "%s does not have enough room.", Name(sdb[x].object));
		} else if (sdb[n].shield.active[0] || sdb[n].shield.active[1] || sdb[n].shield.active[2] || sdb[n].shield.active[3] || sdb[n].shield.active[4] || sdb[n].shield.active[5]) {
			notify_format(enactor, "%s must lower all shields to land.", Name(sdb[n].object));
		} else if (sdb2dissipation(x, sdb2shield(x, n)) != 0.0) {
			notify_format(enactor, "%s's facing shield must be lowered.", Name(sdb[x].object));
		} else if (sdb[x].status.tractored) {
			notify_format(enactor, "%s is being tractored.", Name(sdb[x].object));
...
int do_set_launch (dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s is not on a landing pad.", Name(sdb[n].object));
	} else if (sdb[n].status.connected) {
		notify_format(enactor, "%s is still connected.", Name(sdb[n].object));
	} else if (!SpaceObj(sdb[l].object) || !GoodObject(sdb[l].object)) {
...
int do_set_enter_wormhole (int contact, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify_format(enactor, "%s is much too large to enter.", Name(sdb[n].object));
	} else if (sdb[n].status.docked) {
		notify_format(enactor, "%s is in dock.", Name(sdb[n].object));
	} else if (sdb[n].status.landed) {
		notify_format(enactor, "%s is on a landing pad.", Name(sdb[n].object));
		...
		notify_format(enactor, "%s cannot gate while at warp.", Name(sdb[n].object));
	} else {
		x = contact2sdb(n, contact);
		if (error_on_contact(n, x, enactor)) {
			return 0;
			...
			notify_format(enactor, "%s is moving too fast.", Name(sdb[x].object));
			...
			if (t) {
				if (sdb[t].structure.type != 1) {
					notify_format(enactor, "%s is much too large to enter.", Name(sdb[t].object));
					return 0;
				} else if (sdb2range(t, x) > MAX_WORMHOLE_DISTANCE) {
					notify_format(enactor, "%s is too far away to enter.", Name(sdb[t].object));
					return 0;
				} else
					up_wormhole(t, x);
				}
...
int do_set_fix_damage (char *sys1, char *sys2, int type, char *name, dbref enactor)
{
	...
	if (!GoodSDB(n)) {
		...
		notify_format(enactor, "%s has no repair capacity.", Name(sdb[n].object));
		return 0;
	}
	...
	if (*dmg >= 1.0) {
		notify_format(enactor, "%s on %s has no damage to repair.",
		  system_name[num], Name(sdb[x].object));
		*dmg = 1.0;
		return 0;
	} else if ((*dmg <= -1.0) && (!sdb[n].structure.has_docking_bay) && num) {
		notify_format(enactor, "%s on %s has been totally destroyed.",
		  system_name[num], Name(sdb[x].object));
		return 0;
	}
...
int do_set_refuel (char *name, char *fuel, double tons, dbref enactor)
{
	...
	if (x == SENSOR_FAIL) {
		...
		if (amount > available) {
			notify_format(enactor, "%s has only %f tons of antimatter available",
				Name(sdb[n].object), available / 1000000.0);
		} else if (capacity <= 0) {
			notify_format(enactor, "%s has no antimatter capacity",
				Name(sdb[x].object));
		} else if (amount > capacity) {
			notify_format(enactor, "%s has only %f tons of antimatter capacity",
		...
		if (amount > available) {
			notify_format(enactor, "%s has only %f tons of deuterium available",
				Name(sdb[n].object), available / 1000000.0);
		} else if (capacity <= 0) {
			notify_format(enactor, "%s has no deuterium capacity",
				Name(sdb[x].object));
		} else if (amount > capacity) {
			notify_format(enactor, "%s has only %f tons of deuterium capacity",
				Name(sdb[x].object), capacity / 1000000.0);
		} else {
		...
		if (amount > available) {
			notify_format(enactor, "%s has only %f GW hours of reserves available",
				Name(sdb[n].object), available / 3600.0);
		} else if (capacity <= 0) {
			notify_format(enactor, "%s has no reserve capacity",
				Name(sdb[x].object));
		} else if (amount > capacity) {
			notify_format(enactor, "%s has only %f GW hours of reserves capacity",
...
int do_set_defuel (char *fuel, double tons, dbref enactor)
{
	...
	if (GoodSDB(x)) {
		if (!sdb[n].status.connected) {
			notify_format(enactor, "%s is not connected.", Name(sdb[n].object));
		} else if (f == 1) {
			amount = tons * 1000000.0;
			available = sdb[n].fuel.antimatter;
			capacity = sdb2max_antimatter(x) - sdb[x].fuel.antimatter;
	...
	if (error_on_console(enactor)) {
		...
		if (!flag) {
		  ...
		  sdb[n].coords.y && sdb[n].coords.zd == sdb[n].coords.z) {
			notify_format(enactor, "%s is already there.", Name(sdb[n].object));
		} else {
