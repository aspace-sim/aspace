...
#define SpaceObj(x)                                                            \
  (has_flag_by_name(x, "SPACE-OBJECT", TYPE_THING | TYPE_PLAYER))
...
/**
        ...
        space_log.c will now be capable of the following:-
                ...
                * writing to a channel (falling back on the logfile if the
channel doesn't exist)
                * write to both the channel and the logfile
**/
#define LOG_TYPE                                                               \
  3 // 1 = write to log file, 2 = send to log channel, 3 = do both 1 and 2
...
extern void addNewBorder(dbref executor, int border_number, const char *name,
                         int space, double radius, double x, double y, double z,
                         char *buff, char **bp);
extern void deleteBorder(dbref executor, int border, char *buff, char **bp);
extern void list_borders(char *buff, char **bp);
extern void edit_border(dbref executor, int border_id, const char *setting,
                        const char *new_value, char *buff, char **bp);
...
extern int convert_weapons(dbref ship, dbref executor, char *weaponType,
                           char *weaponCountAttr, char *newAttr, char *oldAttr);
...
extern double xyz2range(double xa, double ya, double za, double xb, double yb,
                        double zb);
...
/* from space_notify.c */
extern void do_console_notify(int x, dbref parent1, dbref parent2,
                              dbref parent3, const char *msg);
extern void do_all_console_notify(int x, const char *msg);
extern void do_ship_notify(int x, const char *msg);
extern void do_space_notify_one(int x, dbref parent1, dbref parent2,
                                dbref parent3, const char *msg);
extern void do_space_notify_two(int n1, int n2, dbref parent1, dbref parent2,
                                dbref parent3, const char *msg);
...
/* from move.c */
extern void moveit(dbref what, dbref where, int nomovemsgs, dbref enactor,
                   const char *cause);
...
/* from space_get.c */
extern void do_space_db_get(int x, char *f1, char *f2, char *f3, char *f4,
                            char *buff, char **bp);

/* from space_put.c */
extern void do_space_db_put(int x, char *value, char *f1, char *f2, char *f3,
                            char *f4, char *buff, char **bp);

/* from space_misc.c */
extern void dump_space(dbref executor);
extern int debug_space(int x);
extern void assess_damage(int n1, int n2, int hits, double damage, double freq,
                          int mode);
...
extern int do_lock_weapon(int first, int last, int contact, int weapon,
                          dbref enactor);
...
extern int do_set_weapon(int first, int last, int active, int weapon,
                         dbref enactor);
...
extern int do_set_missiles_locked(int first, int last, int contact,
                                  dbref enactor);
...
extern int do_set_ftr_alloc(double helm, double tactical, double operations,
                            dbref enactor);
extern int do_set_eng_alloc(double helm, double tactical, double operations,
                            dbref enactor);
extern int do_set_helm_alloc(double movement, double shields, double cloak,
                             dbref enactor);
extern int do_set_shield_alloc(double forward, double starboard, double aft,
                               double port, double dorsal, double ventral,
                               dbref enactor);
extern int do_set_tactical_alloc(double beams, double missiles, double sensors,
                                 dbref enactor);
extern int do_set_sensor_alloc(double ecm, double eccm, dbref enactor);
extern int do_set_operations_alloc(double transporters, double tractors,
                                   double miscellaneous, dbref enactor);
...
extern int do_set_fix_damage(char *sys1, char *sys2, int type, char *name,
                             dbref enactor);
...
extern int do_set_fire(int first, int last, int weapon, int mode,
                       dbref enactor);
extern int do_set_intercept(int contact, dbref enactor);
...
extern void do_sensor_contacts_bot(char *a, dbref enactor, char *buff,
                                   char **bp);
