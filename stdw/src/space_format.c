...
char *format_s_Shield (int x)
{
	...
	for (i = 0 ; i < MAX_SHIELD_NAME ; ++i) {
		...
		if (!sdb[x].shield.active[i]) {
			...
			strncat(buffer, tprintf("/%s\n",
			  unparse_power(sdb[x].shield.maximum * 20.0)), sizeof(buffer) - 1);
		} else {
			strncat(buffer, tprintf("%10.10s: %5.5s %s\n",
			  unparse_power(d), unparse_percent(sdb[x].shield.capacitor[i] / ( sdb[x].shield.maximum * 20.0 )),
			  ansi_stoplight_scale(sdb[x].shield.capacitor[i] / ( sdb[x].shield.maximum * 20.0 ), 20)), sizeof(buffer) - 1);
		}
	}
